...
public class HibernateF {
    ...
    public void addSchool(Schools sc) {
     ...
     public Schools getUserById(int scid) {
        Schools sc = null;
        Transaction trns = null;
        Session session = HibernateUtil.getSessionFactory().openSession();
        try {
            trns = session.beginTransaction();
            String queryString = "from Schools where id = :id";
            Query query = session.createQuery(queryString);
            query.setInteger("id", scid);
            sc = (Schools) query.uniqueResult();
        } catch (RuntimeException e) {
            e.printStackTrace();
        } finally {
            session.flush();
       ...
       public void updateSc(Schools sc) {
        Transaction trns = null;
        Session session = HibernateUtil.getSessionFactory().openSession();
        try {
            trns = session.beginTransaction();
